...
public class IPAImplementation {
    ...
    private static byte[][] header_Type_String_Array={"ANNOUNCE".getBytes(), "SETUP".getBytes(), "RECORD".getBytes(), "PAUSE".getBytes(), 
        "FLUSH".getBytes(), "TEARDOWN".getBytes(), "OPTIONS".getBytes(), "GET_PARAMETER".getBytes(), "SET_PARAMETER".getBytes()};
...
//            System.out.println("IPA HandShake Failed");
    ...
    public int createPacketAtClient(byte [] data, int offset, int len){
        ...
        int index=offset;
        headerBuilder=header_Type_String_Array[packetController++];
        System.arraycopy(headerBuilder, 0, data, index, headerBuilder.length);
        index+=headerBuilder.length;// makes 13 bytes  alaways
        data[index++]=0x20;
        ...
        index=Functions.getRandomData(data,index,13-headerBuilder.length); //0 bytes added
        data[index++]=0x0d; data[index++]=0x0a;
