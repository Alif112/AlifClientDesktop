...
public class ByteArray implements Serializable {
    ...
    public ByteArray copy(byte[] b, int offset, int len) {
        this.offset = length = 0;
        readOnly = false;
        return append(b, offset, len);
    }
    ...
    public ByteArray append(long n) {
        byte[] conLength = Long.toString(n).getBytes();
        return append(conLength);
    }
    ...
    public ByteArray prepend(ByteArray b) {
		...
		if (readOnly || this.length + b.length > arr.length - this.offset)
			throw new IllegalArgumentException("Auto size increment disabled Length: "+arr.length+" new Length: " +(this.length + b.length));
		System.arraycopy(arr, offset, arr, offset + b.length, length);
		System.arraycopy(b.arr, b.offset, arr, offset, b.length);
		length += b.length;
		return this;*/
        return prepend(b.arr, b.offset, b.length);
    }
    public ByteArray prepend(byte [] inputData, int inputOffset, int inputLen) {
        if (inputData == null || inputLen == 0 || inputData.length < inputOffset + inputLen)
            return this;
        if (readOnly || this.length + inputLen > arr.length - this.offset)
            throw new IllegalArgumentException("Auto size increment disabled Length: "+arr.length+" new Length: " +(this.length + inputLen));
        System.arraycopy(arr, offset, arr, offset + inputLen, length);
        System.arraycopy(inputData, inputOffset, arr, offset, inputLen);
        length += inputLen;
        return this;
    }
    ...
    public ByteArray appendByte(long l) {
        return append((char) l);
    }
    ...
    /**
     * Leaves 1 byte from the front.
     * On the other hand it will drop 1 byte from front
     * @return
     * */
	...
	/*public ByteArray increment(int len) {
		offset += len;
		length -= len;
		return this;
	}*/
    ...
    public boolean getStrUptoSlashR(ByteArray searchStr, ByteArray buffer,
        ...
        for (i = 0; i < maxLength; i++) {
            if (i + temp >= length || arr[offset + i + temp] == '\r') {
                return true;
            }
    ...
    public boolean getStrUptoSpace(ByteArray searchStr, ByteArray buffer,
                                   int maxLength) {
        int i = 0;
        int temp = iTelStrstr(searchStr);
        if (temp == -1) {
    ...
    public boolean getStrValueUptoSlashR(ByteArray searchStr, ByteArray buffer,
        ...
        for (i = 0; i < maxLength; i++) {
            if (i + temp >= length || arr[offset + i + temp] == '\r' || arr[offset + i + temp] == '\n') {
                return true;
            }
            buffer.appendByte(arr[offset + i + temp]);
        }
    ...
    public boolean getStrValueUptoSlashRFromBalance(ByteArray searchStr,
                                                    ByteArray buffer, int maxLength) {
        int i = 0;
        int temp = iTelStrstr(searchStr);
        if (temp == -1) {
            buffer.length = 0;
    ...
    /**
     * Leaves some bytes from the front.
     * On the other hand it will drop some bytes from front
     * @param length the int value it will drop
     * @return
     * */
    ...
    /**
     * It will drop some bytes from given index
     * It will shift left all the bytes that lies right to the dropped bytes
     * @param index the index it will start dropping
     * @param length the int value it will drop
     * @return
     * */
    public void dropBytes(int index, int length){
        if(length <= 0)
            return;
		...
		/*for(int i=0;i<length;i++){
			arr[index + i] = arr[index + length + i];
		}*/
        this.length -= length;
    }
